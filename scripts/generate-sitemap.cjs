...
const locationJson = locationText.slice(locationText.indexOf('export const locationPages'));
const locationSlugs = [...locationJson.matchAll(/^\s{4}slug:\s*["']([^"']+)["']/gm)].map(m => m[1]);
...
const servicesJson = servicesText.slice(servicesText.indexOf('export const serviceCategories'));
let currentCategory = null;
servicesJson.split(/\r?\n/).forEach(line => {
  const catMatch = line.match(/^\s{4}slug:\s*["']([^"']+)["']/);
  if (catMatch) currentCategory = catMatch[1];
  const serviceMatch = line.match(/^\s{8}slug:\s*["']([^"']+)["']/);
